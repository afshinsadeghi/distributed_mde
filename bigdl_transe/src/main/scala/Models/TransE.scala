...
object TransE {
  ...
  def CreateEmbedding (entityListLength:Int, relationListLength:Int, k:Int) : (Tensor[Float], Tensor[Float]) = {  
    ...
    //Creates the embedding and save it to the resource folder
  def CreateEmbedding (entityListLength:Int, relationListLength:Int, k:Int, resourceDir:String) : (Tensor[Float], Tensor[Float]) = {  
    val entityEmbedding   = Tensor(entityListLength, k).rand(-6 / Math.sqrt(k), 6 / Math.sqrt(k))
    val relationEmbedding = Tensor(relationListLength, k).rand(-6 / Math.sqrt(k), 6 / Math.sqrt(k))    

    SaveTrainedEmbedding(entityEmbedding,   resourceDir + "entityEmbedding1.tsv")
    SaveTrainedEmbedding(relationEmbedding, resourceDir + "relationEmbedding1.tsv")
  ...
  def Distance(sample:Array[Row], entityListLength:Integer, entityEmbedding:Tensor[Float], relationEmbedding:Tensor[Float], L:Int, gamma:Float, k:Int): (Tensor[Float]) = {      
    val sampleDistance = Tensor(sample.size, 1)
    var count = 1    
    sample.map(row => {
      
      val subTensor  = entityEmbedding.select  (1, row.get(0).toString.toInt)//.reshape(Array(k,1))
      val predTensor = relationEmbedding.select(1, row.get(1).toString.toInt)//.reshape(Array(k,1))
      val objTensor  = entityEmbedding.select  (1, row.get(2).toString.toInt)//.reshape(Array(k,1))      
  ...
  def L_p_norm(SubTensor:Tensor[Float], predTensor:Tensor[Float], objTensor:Tensor[Float], L:Int): (Tensor[Float]) = {
    L match {
      case 1 => ((SubTensor + predTensor - objTensor).abs) //"/home/tasneem/Desktop/eclipse/workspace/Thesis/src/main/resources/" + datasetName + "/"L1
  ...
  def main (args: Array[String]){
    ...
    val trainSet   = spark.read.option("sep", "\t").csv(resourceDir + "train.tsv").rdd
    val summary    = spark.read.option("sep", "\t").option("header", "true").csv(resourceDir + "summary.tsv").take(1)
    val entityList = spark.read.option("sep", "\t").csv(resourceDir + "entityToID.tsv").select("_c1").rdd.map(r => r(0).asInstanceOf[Object]).collect
    ...
    val (entityEmbedding, relationEmbedding) = CreateEmbedding(entityListLength, relationListLength, k, resourceDir)
    
    /*
    val tempEntityEmbedding   = entityEmbedding.clone
    val tempRelationEmbedding = relationEmbedding.clone    
    println("Before optimizer:")
    println("\nEntity embedding:\n"   +  tempEntityEmbedding + "\n")
    println("\nRelation embedding:\n" +  tempRelationEmbedding+ "\n\n")*/
    ...
    for(epoch <- 1 to nEpoch){ //line 4 of the main algorithm
      ...
      val positiveDistance = Distance(positiveSample, entityListLength, entityEmbedding, relationEmbedding, L, gamma, k) //line 10 of the main algorithm
      val negativeDistance = Distance(negativeSample, entityListLength, entityEmbedding, relationEmbedding, L, gamma, k) //line 10 of the main algorithm       
      //println("Epoch: " + epoch)
    ...
    println("\nEntity embedding:\n"     +  entityEmbedding+ "\n")
    println("\nRelation embedding:\n" +  relationEmbedding + "\n\n")
    println("Difference in embeddings:")
    println("\nEntity embedding:\n"   +  (tempEntityEmbedding   - entityEmbedding) + "\n" )
    println("\nRelation embedding:\n" +  (tempRelationEmbedding - relationEmbedding) + "\n\n") */
    
    SaveTrainedEmbedding(entityEmbedding,   resourceDir + "entityEmbedding.tsv")
    SaveTrainedEmbedding(relationEmbedding, resourceDir + "relationEmbedding.tsv")
