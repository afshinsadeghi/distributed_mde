...
object TrainTransEVersion1 {
  ...
  def Distance(sample:Array[Row], entityListLength:Integer, embedding:Tensor[Float], L:Int, gamma:Float): (Tensor[Float]) = {      
    ...
    sample.map(row => { 
      ...
      //val dist = (L_p_norm(subTensor, predTensor, objTensor, L) - gamma) * -1
  ...
  def ScoreFunction(posDist:Tensor[Float], negDist:Tensor[Float], gamma:Float, k:Int, trainSize: Long): (Float) = {
    println("Pos dist: " + posDist.sum + "\nNeg dist: " + negDist.sum)
    val loss = (ReLU().forward(-posDist + negDist + gamma)).sum //margin rank
    //val loss = (ReLU().forward( negDist - posDist  + gamma)).sum //margin rank
    //println((loss/k)/trainSize)
    println("Loss: " + loss)
    (loss)
  }
  
  def DistmultDist(sample:Array[Row], entityListLength:Integer, embedding:Tensor[Float], L:Int, gamma:Float): (Tensor[Float]) = {      
    val sampleDistance = Tensor(sample.size, embedding.size(2))
    var count = 1    
    sample.map(row => { 
      val subTensor  = embedding.select(1, row.get(0).toString.toInt)
      val predTensor = embedding.select(1, row.get(1).toString.toInt + entityListLength)
      val objTensor  = embedding.select(1, row.get(2).toString.toInt)          
      val dist = (subTensor * predTensor.t * objTensor).sum
      sampleDistance.update(count,dist)
      count += 1
    })    
    (sampleDistance)
  ...
  //Trains the TransE
  def TrainTransE(trainSet: RDD[Row], entityList:Array[Object], entityListLength:Int, relationListLength:Int, embedding: Tensor[Float], L:Int, gamma:Float, k: Int, trainSize: Long): (Float) = {
    ...
    val positiveDistance = Distance(positiveSample, entityListLength, embedding, L, gamma) //line 10 of the main algorithm
    val negativeDistance = Distance(negativeSample, entityListLength, embedding, L, gamma) //line 10 of the main algorithm    
    
    //val positiveDistance = DistmultDist(positiveSample, entityListLength, embedding, L, gamma) //line 10 of the main algorithm
    //val negativeDistance = DistmultDist(negativeSample, entityListLength, embedding, L, gamma) //line 10 of the main algorithm    
    ...
    */
    ...
    for(epoch <- 1 to nEpoch){ //line 4 of the main algorithm
      ...
      def Update(x: Tensor[Float]) = {
        ...
        (TrainTransE(trainSet, entityList, entityListLength, relationListLength, embedding, L, gamma, k, trainSize), x) 
      }
    ...
    SaveTrainedEmbedding(embedding, resoiurceDir + "trainedEmbedding.tsv" )
